    ...
    <div class="justify-content-center d-flex ">
      <div class="col-md-8">
        <form method="POST" action="{{ isset($item)  ?  route('edititem') : route('create') }}" class=" p-3">
          @csrf
          <input id="item" type="class" value=" {{ $item->id ?? '' }} " name="item" hidden>
          ...
          <div class="col py-2">
            ...
            <textarea id="desc" value=" {{ $item->description ?? '' }} " class="flex-grow-1 form-control @error('desc') is-invalid @enderror" name="desc" autofocus> </textarea>
            @error('desc')
            <span class="invalid-feedback" role="alert">
              <strong>{{ $message }}</strong>
            </span>
